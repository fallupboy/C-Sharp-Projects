...
namespace SimpleCalculator
{
    public partial class Form1 : Form
    {
        ...
        /// <summary>
        /// Calculates the given equation and outputh the answer to the user label
        /// </summary>
                ...
                for (int i = 0; i < input.Length; i++)
                {
                    ...
                    else if ("+-*/".Any(c => input[i] == c))
                    {
                        ...
                        if (!leftSide)
                        {
                            ...
                            // Check if we actually have a right side numebr
                            if (operation.RightSide.Length == 0)
                            {
                                // Check the operator is not a minus (as they could be creating a negative number)
                                if (operatorType != OperationType.Minus)
                                    throw new InvalidOperationException($"Operator (+ * / or more than one -) specified without and right side number");
                        ...
                        {
                            // Get the operatop type
                            var operatorType = GetOperationType(input[i]);

                            // Check if we actually have a left side numebr
                            if (operation.LeftSide.Length == 0)
                            {
                                // Check the operator is not a minus (as they could be creating a negative number)
                                if (operatorType != OperationType.Minus)
                                    throw new InvalidOperationException($"Operator (+ * / or more than one -) specified without and left side number");
        ...
        /// <summary>
        /// Calculates a <see cref="Operation"/> and returnes the result
        /// </summary>
        ...
        private string CalculateOperation(Operation operation)
        {
            ...
            if (string.IsNullOrEmpty(operation.LeftSide) || !decimal.TryParse(operation.LeftSide, out left))
                throw new InvalidOperationException($"Left side of the operation was not the number. {operation.LeftSide}");
            ...
            if (string.IsNullOrEmpty(operation.RightSide) || !decimal.TryParse(operation.RightSide, out right))
                throw new InvalidOperationException($"Right side of the operation was not the number. {operation.RightSide}");
